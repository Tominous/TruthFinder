			...
			switch (selectedLang)
			{
				...
				case "php":
					...
					conds.forEach((value, index) =>
					{
						...
						outCond = outCond.slice(0, -3) + ") || ( ";
					});
					// Remove the trailing "|| ( "
					outCond = outCond.slice(0, -5);
					break;
				case "python":
					outCond += "( ";
					// Get each condition
					conds.forEach((value, index) =>
					{
						...
						Object.keys(value).forEach(key =>
						{
							// If this variable should be false add a "not "
							outCond += value[key] == "1" ? "" : "not ";
							// Now add the var name
							outCond += key + " and ";
						})
						// Add brackets and remove the trailing "and "
						outCond = outCond.slice(0, -4) + ") or ( ";
					});
