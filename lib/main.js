...
truthTable = {
	...
	translate: function ()
	{
		...
		else if (conds.length === 1 && Object.values(conds[0]).every(value => { return value === "0" } ))
		{
			...
			outCond += notOp.slice(0,7) != "_after_" ? `${notOp}( ` : "(";
			vars.forEach(value =>
			{
				outCond += `${value} ${orOp} `;
			});
			...
			if(notOp.slice(0,7) == "_after_")
			{
				outCondOld = outCond;
				outCond = "";
				for (var i = 0; i < outCondOld.length; i++) {
					outCond += outCondOld.charAt(i) + notOp.slice(7);
				}
			}
		}
		else
		{
			...
			conds.forEach((value, index) =>
			{
				...
				Object.keys(value).forEach(key =>
				{
					...
					outCond += value[key] != "1" && notOp.slice(0,7) != "_after_" ? notOp : "";
					...
					if(notOp.slice(0,7) == "_after_")
					{
						...
						for (var i = 0; i < oldKey.length; i++) {
							key += oldKey.charAt(i) + notOp.slice(7);
						}
